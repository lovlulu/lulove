...
all_position = response['content']['positionResult']['result']
...
for position in all_position:
    ...
    print(i)
    # 招聘具体连接
    url = 'https://www.lagou.com/jobs/' + str(position['positionId']) + '.html'
    print(url)
    dict_laogou['url'] = url
    ...
    print(position_name)
    dict_laogou['position_name'] = position_name

    # 月薪
    salary = position['salary']
    print(salary)
    dict_laogou['salary'] = salary

    # 工作地点
    district = position['district']
    print(district)
    dict_laogou['district'] = district

    # 公司名称'
    companyFullName = position['companyFullName']
    print(companyFullName)
    dict_laogou['companyFullName'] = companyFullName
    print(end='\n')

    # 将字典加入列表
    list.append(dict_laogou)
    i += 1


# 使用pandas 保存数据
df = pandas.DataFrame(list)
df.to_excel('python职位.xlsx')


# list_laogou = []
# list_laogou.append(u"链接"+","+" 职位 "+","+"月薪 "+","+"地点"+","+" 公司\n")
# # list_laogou.append(u"链接 职位 月薪 地点 公司\n")
# for position in all_position:
#
#     # 招聘具体连接
...
#
#
#     # content = str(url)+' '+str(position_name)+' '+str(salary)+' '+str(district)+ ' '+ str(companyFullName) + "\n"
#     content = str(url)+','+str(position_name)+','+str(salary)+','+str(district)+ ','+ str(companyFullName) + "\n"
#     #
#     list_laogou.append(content)
# with open('tt.txt','a',encoding='utf-8') as f:
#     # 注意 写入文件 这里是 writelines
#     # 先保存txt格式 ，到后面可以更改后缀为 csv 注意逗号的使用
#     f.writelines(list_laogou)
