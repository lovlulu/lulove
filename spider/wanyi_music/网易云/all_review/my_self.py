...
### 抓取某一首歌的全部评论
    ...
    all_comments_list.append(u"用户ID 用户昵称  评论内容\n") # 头部信息
    ...
    for i in range(2):  # 逐页抓取  ---------------------这个位置先爬取两页
        # 获得两个参数
        ...
        for item in json_dict['comments']:
            ...
            nickname = item['user']['nickname'] # 昵称
            # avatarUrl = item['user']['avatarUrl'] # 头像地址
            comment_info = str(userID) + " " + str(nickname)  + " " + str(comment) + "\n"
...
if __name__ == "__main__":
    ...
    url = "http://music.163.com/weapi/v1/resource/comments/R_SO_4_479621078?csrf_token=" #替换为你想下载的歌曲R_SO的链接
    filename = "依然爱你.txt" #修改歌曲名称
    all_comments_list = get_all_comments(url)
    save_to_file(all_comments_list,filename)
    end_time = time.time() #结束时间
    print("程序耗时%.3f秒." % (end_time - start_time))